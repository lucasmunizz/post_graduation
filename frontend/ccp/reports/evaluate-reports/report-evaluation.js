...
document.getElementById("evaluationForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const ccpOpinion = document.querySelector('input[name="ccpOpinion"]:checked').value;

    try {
        const response = await fetch(`http://localhost:8080/api/forms/${formId}/ccp-opinion`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${localStorage.getItem("token")}`
            },
            body: JSON.stringify({ ccpOpinion })
        });
